  ...
  if (this.isModified('assignedAuditors')) {
    if (this.assignedAuditors && this.assignedAuditors.length > 0) {
      ...
      if (nonAuditors.length > 0) {
        throw new Error(`Validation failed: Users [${nonAuditors.map(u => u.name).join(', ')}] do not have the Auditor role.`);
      }
      
      if (users.length !== this.assignedAuditors.length) {
        throw new Error('Validation failed: One or more assigned auditor IDs are invalid.');
      }
    }
  }
});
